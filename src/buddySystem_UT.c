...
void deallocateMemory(MemoryTreeNode* root,int startindex,int size) {
    if(root == NULL)
        return;
    // Decide whether to go left or right in the tree based on startindex
    if(startindex >= root->size/2)
    {
    ...
    // If the current node matches the deallocated block, mark it free
    if(root->size == size && root->startindex == startindex)
    {
        root->free=true;
        return;
    }
